...
File: core/tools/prompt_loader.py
Description: Unified prompt loading utility with language fallback support.
             All agents should use this class to load prompts, ensuring consistent
             encoding handling (UTF-8), language fallback, and error handling.
...
class PromptLoaderError(Exception):
    """Exception raised when loading prompt file fails."""
    pass
...
class PromptLoader:
    ...
    Provides consistent prompt loading across all agents with
    encoding fallback and caching capabilities.
